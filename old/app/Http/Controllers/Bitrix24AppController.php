...
class Bitrix24AppController extends Controller
{
    ...
    {
        Log::channel('bitrix24')->info('=== Incoming connector message ===', [
            'messages_count' => count($data['MESSAGES'] ?? []),
            'raw_data' => $data
        ]);
        
        try {
            // НОВОЕ: Фильтруем приветственные сообщения от Битрикс24
            $messages = $data['MESSAGES'] ?? [];
            
            foreach ($messages as $messageData) {
                $messageText = $messageData['message']['text'] ?? '';
                
                // Пропускаем стандартное приветствие Битрикс24
                if (stripos($messageText, 'Добро пожаловать в Открытую линию') !== false && 
                    stripos($messageText, 'Вам ответит первый освободившийся оператор') !== false) {
                    Log::info('Skipping Bitrix24 welcome message');
                    continue;
                }
                
                // Пропускаем автоматические сообщения типа "Меня зовут X, я ваш консультант"
                if (preg_match('/Меня зовут .+, я ваш.+консультант/ui', $messageText)) {
                    Log::info('Skipping Bitrix24 auto-greeting');
                    continue;
                }
            }
