            ...
            if (!empty($result['result'])) {
                $message->update([
                    'metadata' => array_merge($message->metadata ?? [], [
                        ...
                        'bitrix24_sent_message_id' => $result['result'],
    ...
    public function handleOperatorMessage(array $data): void
    {
        try {
            ...
            foreach ($messages as $messageData) {
                ...
                $authorId = $messageData['message']['user_id'] ?? null;
                $authorType = $messageData['user']['type'] ?? null;

                // ИСПРАВЛЕНИЕ: Получаем ID бота из метаданных
                $bitrix24BotId = $conversation->bot->metadata['bitrix24_bot_id'] ?? null;
                
                // Проверяем, не является ли автор сообщения нашим ботом
                if ($bitrix24BotId && $authorId && $authorId == $bitrix24BotId) {
                    Log::info('Ignoring message from our bot', [
                        'bot_id' => $bitrix24BotId,
                        'author_id' => $authorId,
                        'message_preview' => substr($rawText, 0, 50)
                    ]);
                    continue;
                }
                
                // Альтернативная проверка через имя в сообщении
                // Получаем имя бота для проверки
                $botName = $conversation->bot->name ?? null;
                
                // Парсим имя отправителя из сообщения
                $senderName = null;
                if (preg_match('/\[b\](.+?):\[\/b\]/i', $rawText, $matches)) {
                    $senderName = trim($matches[1]);
                    
                    // Если имя отправителя совпадает с именем нашего бота
                    if ($botName && mb_strtolower($senderName) === mb_strtolower($botName)) {
                        Log::info('Ignoring message from bot by name match', [
                            'bot_name' => $botName,
                ...
                // Фильтруем приветственные сообщения
                if (stripos($rawText, 'Добро пожаловать в Открытую линию') !== false) {
                    Log::info('Ignoring Bitrix24 welcome message');
                    continue;
                }
                
                // Проверка на дубликаты по содержимому
                $cleanedText = preg_replace('/\[.*?\]/i', '', $rawText); // Удаляем BB-коды
                ...
                if ($lastAssistantMessage && 
                    trim($lastAssistantMessage->content) === $cleanedText) {
                    Log::info('Skipping duplicate of assistant message', [
                        'original_message_id' => $lastAssistantMessage->id,
                        'duplicate_text_preview' => substr($cleanedText, 0, 50)
                    ]);
                    continue;
                }
                
                // Проверка на дубликаты по ID сообщения Битрикс24
                if ($bitrix24MessageId) {
                    ...
                    if ($exists) {
                        Log::info('Message already exists', ['bitrix24_message_id' => $bitrix24MessageId]);
                        continue;
                    }
                    
                    // Также проверяем, не является ли это ID нашего отправленного сообщения
                    $ourMessage = $conversation->messages()
                        ->where('metadata->bitrix24_message_id', $bitrix24MessageId)
                        ->orWhere('metadata->bitrix24_sent_message_id', $bitrix24MessageId)
                        ->exists();
                        
                    if ($ourMessage) {
                        Log::info('This is our own message echoed back', [
                ...
                // Проверяем тип пользователя из Битрикс24
                if ($authorType === 'BOT' || $authorType === 'CONNECTOR') {
                    Log::info('Skipping message from bot/connector type', [
                ...
                if (preg_match('/\[b\](.+?):\[\/b\]\s*\[br\](.+)/s', $rawText, $matches)) {
                    ...
                    // Проверяем, не бот ли это по имени
                    if ($botName && mb_strtolower($operatorName) === mb_strtolower($botName)) {
                        Log::info('Skipping - operator name matches bot name', [
                    ...
                    // Проверяем на типичные имена ботов
                    $genericBotNames = ['бот', 'bot', 'ассистент', 'assistant', 'виртуальный помощник', 'робот'];
                    
                    $lowerOperatorName = mb_strtolower($operatorName);
                    foreach ($genericBotNames as $genericName) {
                        if (mb_stripos($lowerOperatorName, $genericName) !== false) {
                            Log::info('Skipping - operator name contains bot keyword', [
                                'operator_name' => $operatorName,
                                'matched_keyword' => $genericName
                            ]);
                            continue 2;
                        }
                    }
                    
                    // Если прошли все проверки - это реальный оператор
                    $isRealOperator = true;
                }

                // Создаем сообщение только от реального оператора
                ...
                // Меняем статус только при первом сообщении РЕАЛЬНОГО оператора
                if ($isRealOperator && $conversation->status === 'active') {
                    ...
                    if ($isFirstOperatorMessage) {
                        ...
                        Log::info('🔴 REAL OPERATOR JOINED - Bot disabled', [
                            'conversation_id' => $conversation->id,
                            'operator_name' => $operatorName,
                            'author_id' => $authorId
                        ]);
