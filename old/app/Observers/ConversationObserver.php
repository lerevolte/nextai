...
class ConversationObserver
{
    ...
    {
        // НЕ пропускаем синхронизацию - она нужна после первого реального сообщения
        Log::info("Conversation {$conversation->id} created");
     ...
     */
    private function syncWithBitrix24Connector(Conversation $conversation, $integration): void
    {
        Log::info('syncWithBitrix24Connector', ['conversation_id' => $conversation->id]);
        ...
        // НОВОЕ: Проверяем, нет ли другого диалога с таким же external_id который уже синхронизирован
        $existingSync = Conversation::where('bot_id', $conversation->bot_id)
            ->where('channel_id', $conversation->channel_id)
            ->where('external_id', $conversation->external_id)
            ->where('id', '!=', $conversation->id)
            ->whereNotNull('metadata->bitrix24_chat_id')
            ->first();
            
        if ($existingSync) {
            Log::warning('Another conversation already synced with same external_id', [
                'current_id' => $conversation->id,
                'existing_id' => $existingSync->id,
                'external_id' => $conversation->external_id
            ]);
            
            // Опционально: закрываем дубликат
            $conversation->update([
                'status' => 'closed',
                'closed_at' => now(),
                'metadata' => array_merge($conversation->metadata ?? [], [
                    'closed_reason' => 'duplicate',
                    'original_conversation_id' => $existingSync->id
                ])
            ]);
